...
const KOUKOKU_SYOUKIN_KOUJO_KINGAKU = 500000;

export default function(originZeikomi: number): Kingaku {
  const zeikomi = new BN(originZeikomi);

  let zei;
  if (zeikomi.lte(KOUKOKU_SYOUKIN_KOUJO_KINGAKU)) {
    zei = new BN(0);
  } else {
    zei = zeikomi.minus(KOUKOKU_SYOUKIN_KOUJO_KINGAKU).times(0.1021).floor();
  }
